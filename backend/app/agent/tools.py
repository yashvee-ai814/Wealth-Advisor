...
class ProjectedPotInput(BaseModel):
    ...
    annual_growth_rate: float = Field(gt=0, le=1, description="Expected annual growth rate as decimal, e.g. 0.05 for 5%")
...
class DrawdownIncomeInput(BaseModel):
    ...
    state_pension_annual: float = Field(ge=0, description="Annual UK state pension in GBP (0 if not yet eligible)")
...
# ---------------------------------------------------------------------------
# Tool definitions — deterministic Python formulas, no LLM arithmetic
# ---------------------------------------------------------------------------
...
def calculate_projected_pot(
    ...
    return ProjectedPotOutput(
        ...
        total_growth=round(projected_pot - total_contributions, 2),
    ).model_dump()
...
def calculate_drawdown_income(
    ...
    return DrawdownIncomeOutput(
        annual_income=round(drawdown_from_pot + state_pension_annual, 2),
...
def calculate_monthly_savings_needed(
    ...
    monthly = max(0.0, remaining * r / (growth_factor - 1) / 12) if remaining > 0 else 0.0
...
def calculate_shortfall(
    ...
    return ShortfallOutput(
        shortfall=round(max(0.0, diff), 2),
        surplus=round(max(0.0, -diff), 2),
        is_on_track=diff <= 0,
    ).model_dump()
...
def calculate_readiness_score(
    ...
    Use this as the final summary metric after all other calculations are complete.
...
def calculate_inflation_adjusted_goal(
    ...
    """
    adjusted = current_goal * ((1 + inflation_rate) ** years)
    return InflationAdjustedGoalOutput(
        adjusted_goal=round(adjusted, 2),
        inflation_uplift=round(adjusted - current_goal, 2),
    ).model_dump()
...
def get_uk_state_pension_info(
    ...
    FULL_ANNUAL = 11502.0
    eligible = retirement_age >= STATE_PENSION_AGE
    return StatePensionOutput(
        annual_state_pension=FULL_ANNUAL if eligible else 0.0,
        eligible_from_age=STATE_PENSION_AGE,
        years_until_eligible=max(0, STATE_PENSION_AGE - current_age),
        note=(
            f"Full new UK state pension of £{FULL_ANNUAL:,.0f}/yr from age {STATE_PENSION_AGE}."
            if eligible
            else f"No state pension at planned retirement age {retirement_age}. Starts at age {STATE_PENSION_AGE}."
        ),
    ).model_dump()
...
def ask_human(question: str) -> str:
    ...
    Do NOT guess missing values — always ask instead.
    """
