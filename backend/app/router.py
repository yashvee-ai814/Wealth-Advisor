...
def _guardrail(message: str) -> tuple[bool, str]:
    ...
    for p in _INJECTION_PATTERNS:
        if p in lower:
            return False, "I can only assist with UK wealth planning and retirement questions."
    ...
    if not any(h in lower for h in _WEALTH_HINTS) and len(message.split()) > 8:
...
def _check_ai_output(reply: str) -> bool:
    lower = reply.lower()
    return not any(p in lower for p in ["ignore previous", "new instructions:", "[system]", "<script>"])
...
async def chat(req: ChatRequest) -> ChatResponse:
    ...
    # Extract tool calls used this turn
    ...
    # Determine status and pending interrupt
    status = "complete"
