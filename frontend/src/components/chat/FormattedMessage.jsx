function parseInline(text, keyPrefix = '') {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[2]) {
      parts.push(<strong key={`${keyPrefix}-b-${match.index}`} className="font-semibold text-amber-700 dark:text-amber-300">{match[2]}</strong>)
    } else if (match[3]) {
      parts.push(<em key={`${keyPrefix}-i-${match.index}`} className="italic text-slate-600 dark:text-slate-300">{match[3]}</em>)
    } else if (match[4]) {
      parts.push(<code key={`${keyPrefix}-c-${match.index}`} className="bg-slate-200 dark:bg-slate-700 text-amber-700 dark:text-amber-300 px-1.5 py-0.5 rounded font-mono text-xs">{match[4]}</code>)
    }
...
function AssessmentBlock({ data }) {
  const labelColour = {
    'On track':       'text-emerald-700 bg-emerald-50  border-emerald-300 dark:text-emerald-400 dark:bg-emerald-950 dark:border-emerald-700',
    'Needs attention':'text-amber-700   bg-amber-50    border-amber-300   dark:text-amber-400   dark:bg-amber-950   dark:border-amber-700',
    'At risk':        'text-red-700     bg-red-50      border-red-300     dark:text-red-400     dark:bg-red-950     dark:border-red-700',
  }[data.readiness_label] ?? 'text-slate-700 bg-slate-100 border-slate-300 dark:text-slate-300 dark:bg-slate-800 dark:border-slate-600'
    ...
    <div className="space-y-3 mt-1">
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <div key={label} className="bg-slate-50 border border-slate-200 dark:bg-slate-800/80 dark:border-slate-700 rounded-xl p-3 text-center">
      ...
      {data.action_steps?.length > 0 && (
        <div className="space-y-2">
          ...
          {data.action_steps.map((step, i) => {
            ...
            return (
              <div key={i} className="flex gap-2 items-start bg-slate-50 dark:bg-slate-800/50 rounded-lg px-3 py-2">
...
function renderLines(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('### ')) {
      ...
      elements.push(<h3 key={key++} className="text-sm font-semibold text-amber-600 dark:text-amber-300 mt-3 mb-1">{line.slice(4)}</h3>)
    } else if (line.startsWith('## ')) {
      flushBullets(); flushNumbered()
      elements.push(<h2 key={key++} className="text-base font-bold text-amber-700 dark:text-amber-400 mt-3 mb-1">{line.slice(3)}</h2>)
    } else if (line.startsWith('# ')) {
      flushBullets(); flushNumbered()
      elements.push(<h1 key={key++} className="text-lg font-bold text-amber-700 dark:text-amber-400 mt-3 mb-1 font-display">{line.slice(2)}</h1>)
    } else if (/^[-*] /.test(line)) {
      flushNumbered(); bulletBuf.push(line.slice(2))
    } else if (/^\d+\. /.test(line)) {
      flushBullets(); numberedBuf.push(line.replace(/^\d+\. /, ''))
    } else if (line.trim() === '') {
      flushBullets(); flushNumbered()
