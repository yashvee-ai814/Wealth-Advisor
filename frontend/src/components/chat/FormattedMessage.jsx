function parseInline(text, keyPrefix = '') {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[2]) parts.push(<strong key={`${keyPrefix}-b-${match.index}`} className="font-semibold text-amber-300">{match[2]}</strong>)
    else if (match[3]) parts.push(<em key={`${keyPrefix}-i-${match.index}`} className="italic text-slate-300">{match[3]}</em>)
    else if (match[4]) parts.push(<code key={`${keyPrefix}-c-${match.index}`} className="bg-slate-700 text-amber-300 px-1.5 py-0.5 rounded font-mono text-xs">{match[4]}</code>)
...
function AssessmentBlock({ data }) {
  ...
  const scoreColour = data.readiness_score >= 70
    ? 'text-emerald-400'
    : data.readiness_score >= 40 ? 'text-amber-400' : 'text-red-400'
